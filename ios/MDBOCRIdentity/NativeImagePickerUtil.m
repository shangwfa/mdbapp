...
#import "NativeImagePickerUtil.h"
...
@implementation ETImagePickerModule
...
RCT_EXPORT_METHOD(takeImagePicker:(NSDictionary *)options resolver:(RCTPromiseResolveBlock)resolver reject:(RCTPromiseRejectBlock)reject){
  self.options = options;
  dispatch_async(dispatch_get_main_queue(), ^{
    TakePhotoViewController *vc = [TakePhotoViewController new];
    NSString *title = [self.options objectForKey:@"title"];
    NSString *tip = [self.options objectForKey:@"titleTips1"];
    NSString *subTip = [self.options objectForKey:@"titleTips2"];
    vc.title = title;
    vc.tip = tip;
    vc.subTip = subTip;
    vc.resolveBlock = resolver;
    vc.rejectBlock = reject;
    UIWindow *window = [[[UIApplication sharedApplication] delegate] window];
    UINavigationController *rootNav = (UINavigationController*)window.rootViewController;
    [rootNav presentViewController:vc animated:YES completion:^{
      
    }];
  });
  
}
RCT_EXPORT_METHOD(takeImagePickerSys:(NSDictionary *)options callback:(RCTResponseSenderBlock)callback)
{
  self.callback = callback; // Save the callback so we can use it from the delegate methods
  self.options = options;
    ...
    if ([mediaType isEqualToString:(NSString *)kUTTypeImage]) { // PHOTOS
      ...
      if (imageURL) {
        ...
        if (pickedAsset.creationDate) {
          self.response[@"timestamp"] = [[ETImagePickerModule ISO8601DateFormatter] stringFromDate:pickedAsset.creationDate];
        }
      }
            ...
            if (error) {
              ...
              if (assetURL) {
                ...
                if (capturedAsset.creationDate) {
                  self.response[@"timestamp"] = [[ETImagePickerModule ISO8601DateFormatter] stringFromDate:capturedAsset.creationDate];
                }
              }
