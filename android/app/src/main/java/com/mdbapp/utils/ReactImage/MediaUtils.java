...
import static com.mdbapp.module.ETImagePickerModule.REQUEST_LAUNCH_IMAGE_CAPTURE;
...
public class MediaUtils
{
                       ...
                       @NonNull final boolean forceLocal)
    {
        ...
        final File path = ReadableMapUtils.hasAndNotNullReadableMap(options, "storageOptions") && ReadableMapUtils.hasAndNotEmptyString(options.getMap("storageOptions"), "path")
    ...
    public static void removeUselessFiles(final int requestCode,
                                          @NonNull final ImageConfig imageConfig)
    {
        if (requestCode != ETImagePickerModule.REQUEST_LAUNCH_IMAGE_CAPTURE)
        {
            return;
        }
